...
import { Shield, FileText, AlertTriangle, Users, EnvironmentBuilding, X, CheckCircle } from "./Icons";
// Tambahkan ikon untuk modal
import { ClipboardList, BarChart, Settings, RefreshCw } from "./Icons";

// Komponen Modal untuk Manajemen Risiko
const ManajemenRisikoModal = ({ isOpen, onClose }) => {
  if (!isOpen) return null;

  const prosesManajemenRisiko = [
    {
      id: 1,
      title: "Identifikasi Bahaya",
      description: "PT. Siantar Top secara sistematis mengidentifikasi semua potensi sumber bahaya di lingkungan kerja, termasuk area produksi, gudang, laboratorium, dan fasilitas pendukung lainnya.",
      icon: ClipboardList,
      color: "text-blue-600",
      bgColor: "bg-blue-100",
      contoh: [
        "Pemeriksaan rutin mesin produksi",
        "Analisis bahan kimia berbahaya (B3)",
        "Identifikasi titik-titik rawan kecelakaan",
        "Review proses kerja di semua departemen"
      ]
    },
    {
      id: 2,
      title: "Penilaian Risiko",
      description: "Menilai kemungkinan terjadinya risiko dan tingkat keparahan akibatnya menggunakan matriks risiko 5x5 sesuai standar HIRADC (Hazard Identification, Risk Assessment, and Determining Control).",
      icon: BarChart,
      color: "text-amber-600",
      bgColor: "bg-amber-100",
      contoh: [
        "Penilaian risiko kebakaran area penyimpanan",
        "Analisis risiko ergonomi di stasiun kerja",
        "Evaluasi paparan kebisingan mesin produksi",
        "Assesment risiko kontaminasi produk"
      ]
    },
    {
      id: 3,
      title: "Pengendalian Risiko",
      description: "Mengambil tindakan hierarkis untuk mengurangi atau menghilangkan risiko yang telah diidentifikasi, mulai dari eliminasi hingga penggunaan Alat Pelindung Diri (APD).",
      icon: Settings,
      color: "text-emerald-600",
      bgColor: "bg-emerald-100",
      contoh: [
        "Instalasi guarding pada mesin berbahaya",
        "Substitusi bahan kimia berbahaya dengan yang lebih aman",
        "Penerapan prosedur kerja aman (SOP)",
        "Pemasangan signage dan marking area berbahaya"
      ]
    },
    {
      id: 4,
      title: "Pemantauan & Review",
      description: "Memantau efektivitas tindakan pengendalian yang telah diterapkan dan melakukan review berkala terhadap desain pengendalian risiko untuk perbaikan berkelanjutan.",
      icon: RefreshCw,
      color: "text-purple-600",
      bgColor: "bg-purple-100",
      contoh: [
        "Audit internal K3 triwulanan",
        "Review laporan insiden dan near-miss",
        "Update prosedur berdasarkan temuan terbaru",
        "Evaluasi kinerja kontraktor terkait K3"
      ]
    }
  ];

  const dokumenTerkait = [
    "Matriks HIRADC Terkini",
    "Register Identifikasi Bahaya",
    "Laporan Penilaian Risiko Tahunan",
    "Rencana Tindak Pengendalian (Action Plan)"
  ];
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 p-6 rounded-t-2xl z-10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-2xl font-bold text-gray-900">Manajemen Risiko K3</h2>
                <p className="text-gray-600">PT. Siantar Top</p>
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="p-6">
          ...
          <div className="mb-8">
            <p className="text-gray-700 leading-relaxed">
              PT. Siantar Top menerapkan sistem manajemen risiko terstruktur yang mengikuti siklus 
              <span className="font-semibold text-amber-600 mx-1">Identifikasi → Penilaian → Pengendalian → Pemantauan</span>. 
              Sistem ini memastikan semua potensi bahaya di lingkungan kerja dapat dikelola secara proaktif.
            </p>
          </div>

          {/* 4 Proses Manajemen Risiko */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            {prosesManajemenRisiko.map((proses) => (
              <div
                key={proses.id}
                className="border border-gray-200 rounded-xl p-6 hover:shadow-md transition-shadow"
              >
                <div className="flex items-start gap-4">
                  <div className={`p-3 rounded-lg ${proses.bgColor}`}>
                    <proses.icon className={`w-6 h-6 ${proses.color}`} />
                  </div>
                  <div className="flex-1">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-gray-700 mb-2">
                        Contoh di PT. Siantar Top:
                      </p>
                      <ul className="space-y-1">
                        {proses.contoh.map((item, idx) => (
          ...
          <div className="bg-gray-50 rounded-xl p-6">
            <h4 className="text-lg font-bold text-gray-900 mb-4">
              Dokumen Manajemen Risiko PT. Siantar Top
            </h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {dokumenTerkait.map((dokumen, index) => (
        ...
        <div className="border-t border-gray-200 p-6">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            <div className="text-sm text-gray-500">
              ...
              <p className="text-xs mt-1">Sistem dikelola oleh: Divisi K3 PT. Siantar Top</p>
            </div>
...
// Main Component dengan State untuk Modal
export const ManagementSection = ({ onOpenSMK3, onOpenOrgChart, onOpenBlueprint, onOpenAudit }) => {
  const [isRisikoModalOpen, setIsRisikoModalOpen] = useState(false);

  return (
    <>
      <section id="manajemen" className="py-24 bg-gradient-to-b from-white to-gray-50 w-full">
        <Container>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
            ...
            <ManagementItem
              ...
              description="Siklus identifikasi, penilaian, pengendalian, dan pemantauan risiko K3."
              Icon={AlertTriangle}
              colorClass="bg-amber-600"
              action={() => setIsRisikoModalOpen(true)}
            />
            <ManagementItem
      ...
      {/* Modal untuk Manajemen Risiko */}
      <ManajemenRisikoModal
        isOpen={isRisikoModalOpen}
        onClose={() => setIsRisikoModalOpen(false)}
      />
    </>
  );
};
...
export const ManagementSectionSimple = ({ onOpenSMK3, onOpenOrgChart, onOpenBlueprint, onOpenAudit }) => {
  const [isRisikoModalOpen, setIsRisikoModalOpen] = useState(false);
  ...
  const managementItems = [
    ...
    {
      title: "Manajemen Risiko",
      description: "Siklus HIRADC: Identifikasi hingga pemantauan risiko",
      Icon: AlertTriangle,
      color: "bg-amber-100",
